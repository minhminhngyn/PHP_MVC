...
if (isset($_GET['action']) && $_GET['action'] === 'logout') {
    if (isset($_COOKIE['remember_me'])) {
        ...
        $stmt = $con->prepare("UPDATE TaikhoanND SET Tokenluudangnhap = NULL, Ngayhethantoken = NULL WHERE Tokenluudangnhap = ?");
...
if (isset($_COOKIE['remember_me'])) {
    $token = $_COOKIE['remember_me'];
    $stmt = $con->prepare("SELECT MaTK, Vaitro FROM TaikhoanND WHERE Tokenluudangnhap = ? AND Ngayhethantoken > NOW()");
    $stmt->bind_param("s", $token);
    $stmt->execute();
    $stmt->store_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $con->prepare("SELECT TaikhoanND.MaTK, TaikhoanND.Matkhau, TaikhoanND.TrangthaiTK, 
                                  TaikhoanND.Solansaidangnhap, TaikhoanND.Lanchothulai, 
                                  Nguoidung.Email, TaikhoanND.Vaitro
                           FROM TaikhoanND 
                           JOIN Nguoidung ON TaikhoanND.MaND = Nguoidung.MaND 
                           WHERE TaikhoanND.TenTK = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($trangthaiTK == 0) {
            ...
            if (time() <= $timeLocked) { 
                ...
                // Mở khóa tài khoản
                $stmt = $con->prepare("UPDATE TaikhoanND SET TrangthaiTK = 1, Solansaidangnhap = 0, Lanchothulai = NULL WHERE MaTK = ?");
        ...
        if (password_verify($password, $hashedPassword)) {
            ...
            if ($remember) {
                ...
                $stmt = $con->prepare("UPDATE TaikhoanND SET Tokenluudangnhap = ?, Ngayhethantoken = ? WHERE MaTK = ?");
            ...
            $stmt = $con->prepare("UPDATE TaikhoanND SET Solansaidangnhap = ? WHERE MaTK = ?");
            ...
            if ($solansaidangnhap == 1 || $solansaidangnhap == 2) {
                ...
                $stmt = $con->prepare("UPDATE TaikhoanND SET Lanchothulai = ? WHERE MaTK = ?");
                ...
                $stmt = $con->prepare("UPDATE TaikhoanND SET TrangthaiTK = 0, Lanchothulai = ? WHERE MaTK = ?");
                ...
                $token = bin2hex(random_bytes(16));
                $stmt = $con->prepare("UPDATE TaikhoanND SET Tokenxacnhan = ?, Ngayhethanxacnhan = DATE_ADD(NOW(), INTERVAL 60 MINUTE) WHERE MaTK = ?");
